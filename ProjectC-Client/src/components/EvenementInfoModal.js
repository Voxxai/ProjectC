...
import { faTimes, faMapMarkerAlt, faClock, faCalendar, faUserCircle, faUserAlt, faCalendarDays } from '@fortawesome/free-solid-svg-icons';


function EvenementInfoModal({ isOpen, onRequestClose, event, joined, setJoined, endJoinDate, setRefreshTrigger, reloadEventUsers, eventUsersData }) {
    const { auth } = useAuth();
    ...
    const renderButtons = () => {
        if (endJoinDate == true || Date.now() > new Date(event.Date).setDate(new Date(event.Date).getDate() - 1)) {
            return (
    ...
    return (
        ...
        >
            <div className='flex flex-col gap-y-2'>
                ...
                <span className='text-2xl text-cavero-purple font-semibold'>{event ? event.title : onRequestClose}</span>
                </div>
                <div className='flex flex-row'>
                ...
                <div className='flex flex-row gap-x-3 items-center '>
                    ...
                    <span className='text-md text-gray-700 font-semibold'>{event.time.split(':').slice(0, 2).join(':')}</span>
                </div>
                <div className='flex flex-row gap-x-3 items-center '>
